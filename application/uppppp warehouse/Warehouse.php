...
class Warehouse extends CI_Controller {
    ...
    public function common_view($action,$date=0)
    {
        ...
        if(!empty($_SESSION[SESSION_NAME]['getMenus']))
        {
            ...
            $data = array('dateinfo'=>$date,'breadcrumbs' => $breadcrumbs ,'actioncolumn' => '5' ,'ajax_manage_page' => $action,'heading' => 'Manage Warehouse','branch_data'=>$branch_data,'import' =>$import,'importaction' =>$importaction,'download' =>$download,'addPermission'=>$add,'export' =>$export, 'exportPermission'=>$exportbutton);
    ...
    public function listpdf($date = 0)
    {
        ...
        $data['rows'] = $this->Warehouse_model->get_datatables($con,$date);
        $html = $this->load->view('Warehouse/pdf_Warehouse_list',$data,true);
        $mpdf = new \Mpdf\Mpdf();
        $mpdf->WriteHTML($html);
        $mpdf->Output('Warehouse_Return','I');
    }
    ...
    public function export_dispatch1($date = 0)
    {
        ...
        $results = $this->Warehouse_model->get_datatables($con,$date);
        //$results = $this->Warehouse_model->ExportCSV($con);
        ...
        $this->excel->getActiveSheet()->setCellValue('A1', 'Warehouse Summary Details ');
        ...
        {
            $date = date('d-m-Y', strtotime($result->warehouse_date));
    ...
    public function create()
    {
        ...
        $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='" . site_url('Products') . "'>Manage Dispatch Note/Bill</a></li>
...
//        $categories = $this->Crud_model->GetData('categories', "", "status='Active'", '', 'title asc');
//
//        $users = $this->Crud_model->GetData("employees","","id!='".$_SESSION['ASSETSTRACKING']['id']."' and type='User'");
//        $action =  site_url("Warehouse/create_action");
//
...
//    }
    public function save_next($id)
    {
        ...
        $arrDispatch = $this->Crud_model->GetData('warehouse','',$condition,'','','',1);
        $condition2 = array('d.warehouse_id'=>$id);
        //$arrDispatchData = $this->Crud_model->GetData('warehouse_details','',$condition2);
        $query = $this->db
    ...
    public function create_action()
    {
        ...
        if ($_POST) {
            //$subcategory = $this->Crud_model->GetData('categories','',"title='".$_POST['category_id']."'",'','','','1');

            //$bill_date = str_replace('/', '-', $_POST['bill_date']);
            $bill_date = date("Y-m-d", strtotime($_POST['bill_date']));

            //$purchase_date = str_replace('/', '-', $_POST['purchase_date']);
            $purchase_date = date("Y-m-d"); //, strtotime($_POST['purchase_date']));

            //print_r($bill_date); echo "<br/>";
            ...
            if (count($_POST['sku']) > 1) {
                //print_r($_POST);exit;

                for ($i = 0; $i < count($_POST['sku']); $i++) {
                    $product_purchase_date = date("Y-m-d", strtotime($_POST['product_purchase_date'][$i]));
                ...
                $product_purchase_date = date("Y-m-d", strtotime($_POST['date'][0]));
...
//
//                $data_array = array(
//                    'warehouse_date' => date('Y-m-d', strtotime($_POST['date'])),
...
//                    $this->Crud_model->SaveData("warehouse",$data_array,$condition);
//                    $last_id = $did;
//                }
//                else{
//                    $this->Crud_model->SaveData("warehouse",$data_array);
...
//                    $this->Crud_model->SaveData("warehouse_details",$data,$arrCondition);
...
//                if($finish != 'finish') {
//                    return redirect('warehouse/save_next/'.$last_id);
//                } else {
...
//                if($did = $this->input->post('warehouse_id')){
//                    return redirect('warehouse/save_next/'.$did);
//                }
//                else{
...
//    }
    public function edit_action($id,$last_id)
    {
        ...
        $users = $this->Crud_model->GetData("employees","","id!='".$_SESSION['ASSETSTRACKING']['id']."' and type='User'");
        $action =  site_url("Warehouse/create_action/edit");
        $condition = array('id'=>$id);
        $objDis = $this->Crud_model->GetData('dispatch_details','',$condition,'','','',1);
        $condition2 = array('id'=>$last_id);
        $arrDispatchData = $this->Crud_model->GetData('warehouse','',$condition2,'','','',1);
    ...
    public function remove($id,$last_id)
    {
        $condition = array('id'=>$id);
        $objDis = $this->Crud_model->GetData('dispatch_details','',$condition,'','','',1);
        $this->Crud_model->DeleteData('dispatch_details',$condition);
        ...
        //$this->session->set_flashdata('message', '<span class="label label-success text-center" style="margin-bottom:0px">Dispatch/Return has been removed successfully</span>');
        redirect('warehouse/save_next/'.$last_id);
    }
    ...
    public function update($id)
    {
        ...
        $getAssetData = $this->Crud_model->GetData('warehouse',"","id='".$id."'",'','','','row');
        $products = $this->Crud_model->GetData('dispatch_details',"","dispatch_id='".$id."'",'','','','row');
        ...
        $data = array(
            'breadcrumbs' => $breadcrumbs,
            'id' => $id,
            'heading' => 'Update Warehouse/Return',
            'button'=>'Update',
            'action' => $action,
            'getAssetData' => $getAssetData,
            'products' => $products,
    ...
    {
        $con = "id='".$_POST['id']."'";
        $this->Crud_model->DeleteData('assets',$con);
    ...
    public function log_details($id,$branch_id='')
    {
        ...
        $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li><a href='".site_url('Warehouse/index')."'>Manage Warehouse</a></li>
    ...
    public function export_dispatch()
    {
        //print_r($_POST);exit;
    ...
    public function import()
    {
        ...
        if(!isset($fields_fun))
        {
            ...
            redirect(site_url('Warehouse/index'));
        }
        $data = $fields_fun;
        if(count($val) == '8')
        {
    ...
    public function warranty_read($assets_id,$asset_details_id)
    {
        ...
        redirect('Warehouse/viewAssetDetails/'.$assets_id.'/'.$asset_details_id);
    }
    ...
    public function createAssetImages($asset_id='',$id='')
    {
        if(!empty($id))
        {
            ...
            $data = array(
                ...
                'action' =>  site_url('Warehouse/create_actionAssetImages/'.$asset_id.'/'.$id),
            );
    ...
    public function create_actionAssetImages($asset_id,$id)
    {
        if(!empty($id)){
            ...
            if($count > 0)
            {
                ...
                redirect('Warehouse/AssetImagesList/'.$asset_id.'/'.$id);
            }
            else
            {
        ...
        if(!empty($id)){
            $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='".site_url('Warehouse')."'>Manage Warehouse</a></li>
                     <li class=''> <a href='".site_url('Warehouse/view/'.$asset_id)."'>Manage Warehouse View</a></li>
            ...
            $data = array(
                ...
                'ajax_manage_page' => site_url('Warehouse/ajax_manage_page_assets_images/'.$asset_id.'/'.$id) ,
            );
    ...
    public function ajax_manage_page_assets_images($asset_id,$id)
    {
        ...
        foreach ($getData as $Data) {
            ...
            $btn ='&nbsp; '. '<a style="color:#fff" title="Update" href="'.site_url('Warehouse/updateAssetImages/'.$Data->id.'/'.$asset_id.'/'.$id).'"><span class="btn btn-info btn-circle btn-sm"><i class="ace-icon fa fa-pencil bigger-130"></i></a></span>';
    ...
    public function updateAssetImages($id,$asset_id,$asset_details_id)
    {
        ...
        $data = array(
            ...
            'action' => site_url('Warehouse/update_actionAssetImages/'.$id.'/'.$asset_id.'/'.$asset_details_id),
        );
    ...
    public function update_actionAssetImages($id,$asset_id,$asset_details_id)
    {
        ...
        redirect('Warehouse/AssetImagesList/'.$asset_id.'/'.$asset_details_id);
    ...
    public function req_new_asset_listAjax()
    {
        ...
        foreach($Data as $row)
        {
            ...
            $total_assets="<a href=".site_url('Warehouse/assets_request_details/'.$row->id)."><span class='label-info label'>".$row->total_assets."</span></a>";


            if($row->status=='Pending')
            {  $btn='';
                if(!empty($edit)){
                    $btn = ('<a href='.site_url("Warehouse/update_asset_request/".$row->id).' class="btn btn-info btn-circle btn-sm" title="Edit"><i class="ace-icon fa fa-pencil bigger-130"></i></a>');
                }
                if(!empty($delete)){
                    $btn .='&nbsp;|&nbsp;'.'<a href="#deleteData" data-toggle="modal" title="Delete" class="btn btn-danger btn-circle btn-sm" onclick="checkStatus('.$row->id.')"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>';
                }
    ...
    public function req_new_asset()
    {
        $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li>
                        <a href='".site_url('Warehouse/req_new_asset_list')."'>Request New Asset List</a>
                    </li>
                    <li class='active'>Request New Asset</li>
                    </ul>";
        $action =  site_url("Warehouse/new_asset_action");
        $data=array(
    ...
    public function new_asset_action()
    {
        ...
        redirect('Warehouse/req_new_asset_list');
    }
    ...
    public function assets_request_details($request_id)
    {
        if(!empty($request_id))
        {
            ...
            if(!empty($assets_requests))
            {
                $heading=  'Warehouse Request of '.$assets_requests->request_no;
                $breadcrumbs = "<ul class='breadcrumb'>
                        <li>
                            <i class='ace-icon fa fa-home home-icon'></i>
                            <a href='".site_url('Dashboard')."'>Dashboard</a>
                        </li>
                        <li class='active'><a href='".site_url('Warehouse/req_new_asset_list')."'>Request New Product List</a></li>
                        <li class='active'>".$heading."</li>
                        </ul>";
    ...
    public function update_asset_request($request_id)
    {
        if(!empty($request_id))
        {
            ...
            if(!empty($assets_requests))
            {
                ...
                $action =  site_url("Warehouse/update_asset_action/".$request_id);
                $data=array(
