...
class Daily_Sales_Summary_model extends CI_Model
{
     ...
     private function _get_datatables_query($con)
    {
        ...
        $this->db->select(
            ...
            (select sum(discount_1 * rate_per_item * quantity/ 100) from invoice_details where invoice_id=i.id) as discount_1,
            (select sum(discount_2 * rate_per_item * quantity/ 100) from invoice_details where invoice_id=i.id) as discount_2,
            (select sum(discount_3 * rate_per_item * quantity/ 100) from invoice_details where invoice_id=i.id) as discount_3,
