...
class Credit_sales_model extends CI_Model
{
     ...
     private function _get_datatables_query($con)
    {
        ...
        $this->db->where('i.payment_status', 'pending');
        $this->db->where($con);
       /* $i = 0;
    ...
    public function get_settlements($con, $date)
    {
        $this->db->select(
            'ise.id,
            i.invoice_no,
            i.date_of_invoice,
            ise.date as settlement_date,
            (select SUM(net_amount) from invoice_details where invoice_id=i.id) as sumAmount,
            (select type from payment_types where id=i.payment_mode) as paymentMode
            ');
            // a.asset_name,
            // ide.quantity,
            // ide.net_amount,
        $this->db->from("invoice_settlements ise");
        $this->db->join("invoice i","i.id = ise.invoice_id","left");
        // $this->db->join("invoice_details ide","i.id = ise.invoice_id","left");
        // $this->db->join("assets a","a.id = ide.product_id","left");
        $this->db->order_by('ise.date', "desc");

        $this->db->where("ise.invoice_id = i.id");
        $this->db->where('i.created_by',$_SESSION[SESSION_NAME]['id']);
        if($date!=0){
			$dates = explode("_",$date);
			$date1 = date("Y-m-d", strtotime($dates[0]));
			$date2 = date("Y-m-d", strtotime($dates[1]));
			$this->db->where('ise.date >=', $date1);
            $this->db->where('ise.date <=', $date2);
        }
        $query = $this->db->get();
        return $query->result();
    }
