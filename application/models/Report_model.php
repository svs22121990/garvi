...
class Report_model extends CI_Model
{
    ...
    {
        $this->db->select('vp.bank_account_no,vp.cheque_no,vp.cheque_date,vp.bank_name,vp.payment_type,v.name,v.shop_name,e.name as employee_name');
        $this->db->from('vendor_transactions vt');
        $this->db->join('vendor_payments vp','vt.payment_id=vp.id','left');
        $this->db->join('vendors v','vp.vendor_id=v.id','left');
        $this->db->join('employees e','vp.paid_by=e.id','left');
        $this->db->where($con);
