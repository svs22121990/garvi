...
class Warehouse_Products extends CI_Controller {
  ...
  public function approveProduct() { 
    ...
    if($id) {
      ...
      $data_array = array(
        'purchase_date' => $disptach->dispatch_date,
        'bill_no' => $disptach->dn_number,
        'bill_date' => date('Y-m-d H:i:s'),
        'received_from' => $disptach->sent_to,
        'status' => 'Active',
	...
	public function remove($id,$last_id)
	{
		...
		//$this->session->set_flashdata('message', '<span class="label label-success text-center" style="margin-bottom:0px">Dispatch/Return has been removed successfully</span>');
			redirect('dispatch/save_next/'.$last_id); 
	}
    ...
    public function assetDetails($ast_id,$id,$flag)
    {
      ...
      else
      {
       ...
       $breadcrumbs = "<ul class='breadcrumb'>
        ...
        $getAssetData = $this->Dispatch_model->getAllDetails($ast_id);
        ...
        $action=site_url('Dispatch/addDetailsaction/'.$ast_id.'/'.$id.'/'.$flag);
        $data = array(
    ...
    public function update($id)
    {
       $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li class=''> <a href='".site_url('Dispatch')."'>Manage Dispatch</a></li>
                    <li class='active'>Update Asset</li>
                    </ul>";

        $getAssetData = $this->Crud_model->GetData('dispatch',"","id='".$id."'",'','','','row');
        $products = $this->Crud_model->GetData('dispatch_details',"","dispatch_id='".$id."'",'','','','row');
        
        $action =  site_url("Dispatch/update_action/".$id);  
                                       
        $data = array(
          'breadcrumbs' => $breadcrumbs,
          'id' => $id,
          'heading' => 'Update Dispatch/Return',
          'button'=>'Update',                        
          'action' => $action, 
          'getAssetData' => $getAssetData, 
          'products' => $products, 
      ...
      $this->load->view('dispatch/update_form',$data);
  ...
  public function log_details($id,$branch_id='')
  {
    ...
    $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li><a href='".site_url('Dispatch/index')."'>Manage Dispatch</a></li>
    ...
    $data = array(
                  ...
                  'branch_id' => $branch_id,
                  'actioncolumns' => '8' ,
                  'ajax_manage_pages' => site_url('Dispatch/ajax_manage_pages/'.$id.'/'.$branch_id),
                  );
  ...
  else
  {
    $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='".site_url('Dispatch')."'>Manage Dispatch</a></li>
                     <li class=''> <a href='".site_url('Dispatch/view/'.$asset_id)."'>Manage Product View</a></li>
   ...
   $data = array();       
    ...
    { 
     ...
     $barcode_image='-';
      ...
      $btn ='<a href='.site_url("Dispatch/viewAssetDetails/".$row->asset_id.'/'.$row->id).' title="Details" class="btn btn-info btn-circle btn-sm"><i class="fa fa-eye"></i></a>';
      ...
      if(!empty($asset_geo_locations))
      {
         ...
         $btn .=' | <a href="'.site_url("Dispatch/assetDetailGeoLocation/").$row->asset_id.'/'.$row->id.'"  class="btn btn-danger btn-circle btn-sm"><i class="fa fa-map-marker"></i></a>';
      }  
...
public function import()
{
  $file = $_FILES['excel_file']['tmp_name'];
        ...
        if(!isset($fields_fun))
        {
            ...
            redirect(site_url('Dispatch/index'));            
        } 
        $data = $fields_fun;
        if(count($val) == '8')
        {
...
public function warranty_read($assets_id,$asset_details_id)
{
  ...
  redirect('Dispatch/viewAssetDetails/'.$assets_id.'/'.$asset_details_id);
}
...
public function createAssetImages($asset_id='',$id='')
{
  ...
  $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='".site_url('Dispatch')."'>Manage Dispatch</a></li>
                     <li class=''> <a href='".site_url('Dispatch/view/'.$asset_id)."'>Manage Product View</a></li>
                     <li class=''> <a href='".site_url('Dispatch/assetImagesList/'.$asset_id.'/'.$id)."'>Manage Product Images List</a></li>
  ...
  $data = array(
              ...
              'action' =>  site_url('Dispatch/create_actionAssetImages/'.$asset_id.'/'.$id),  
                );
...
{ 
  ...
  { 
    ...
    redirect('Dispatch/AssetImagesList/'.$asset_id.'/'.$id);
  }
  else
  {
...
public function AssetImagesList($asset_id,$id)
{
  ...
  $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='".site_url('Dispatch')."'>Manage Dispatch</a></li>
                     <li class=''> <a href='".site_url('Dispatch/view/'.$asset_id)."'>Manage Dispatch View</a></li>
  ...
  $data = array(
              ...
              'ajax_manage_page' => site_url('Dispatch/ajax_manage_page_assets_images/'.$asset_id.'/'.$id) ,
                );
  ...
  $condition="asset_multiple_images.asset_details_id='".$id."'";
      $getData = $this->Asset_images_model->get_datatables($condition);
        ...
        foreach ($getData as $Data) {
          ...
          $btn ='&nbsp; '. '<a style="color:#fff" title="Update" href="'.site_url('Dispatch/updateAssetImages/'.$Data->id.'/'.$asset_id.'/'.$id).'"><span class="btn btn-info btn-circle btn-sm"><i class="ace-icon fa fa-pencil bigger-130"></i></a></span>';
...
public function updateAssetImages($id,$asset_id,$asset_details_id)
{
  ...
  $data = array(
              ...
              'action' => site_url('Dispatch/update_actionAssetImages/'.$id.'/'.$asset_id.'/'.$asset_details_id),
              );
...
public function update_actionAssetImages($id,$asset_id,$asset_details_id)
{
  ...
  redirect('Dispatch/AssetImagesList/'.$asset_id.'/'.$asset_details_id);
...
public function delete_assets_images($asset_id,$asset_details_id)
{
  ...
  public function req_new_asset_list()
       ...
       $data = array();       
        ...
        {  
           ...
           { 
            ...
            $total_assets="<a href=".site_url('Dispatch/assets_request_details/'.$row->id)."><span class='label-info label'>".$row->total_assets."</span></a>";


           if($row->status=='Pending')    
           {  $btn='';
               if(!empty($edit)){
                 $btn = ('<a href='.site_url("Dispatch/update_asset_request/".$row->id).' class="btn btn-info btn-circle btn-sm" title="Edit"><i class="ace-icon fa fa-pencil bigger-130"></i></a>');
                }
               if(!empty($delete)){
                $btn .='&nbsp;|&nbsp;'.'<a href="#deleteData" data-toggle="modal" title="Delete" class="btn btn-danger btn-circle btn-sm" onclick="checkStatus('.$row->id.')"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>';
               } 
     ...
     public function req_new_asset()
    {
       $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li>
                        <a href='".site_url('Dispatch/req_new_asset_list')."'>Request New Asset List</a>
                    </li>
                    <li class='active'>Request New Asset</li>
                    </ul>";
          $action =  site_url("Dispatch/new_asset_action");             
          $data=array(
    ...
    public function new_asset_action()
    {
       ...
       for($i=0;$i<$asset_name_count;$i++)
      {
        ...
        redirect('Dispatch/req_new_asset_list');
    }
       ...
       public function assets_request_details($request_id)
  {
    ...
    {  
      ...
      if(!empty($assets_requests))
      {  
          $heading=  'Dispatch Request of '.$assets_requests->request_no;   
          $breadcrumbs = "<ul class='breadcrumb'>
                        <li>
                            <i class='ace-icon fa fa-home home-icon'></i>
                            <a href='".site_url('Dashboard')."'>Dashboard</a>
                        </li>
                        <li class='active'><a href='".site_url('Dispatch/req_new_asset_list')."'>Request New Product List</a></li>
                        <li class='active'>".$heading."</li>
                        </ul>";              
        ...
        if(!empty($assets_requests))
        {
          ...
          $assets_request_details = $this->Crud_model->GetData('assets_request_details',"","asset_request_id='".$request_id."' and status='Pending'");
            ...
            $breadcrumbs = "<ul class='breadcrumb'>
                ...
                $action =  site_url("Dispatch/update_asset_action/".$request_id);             
                $data=array(
      ...
      if(!empty($asset_id) && !empty($asset_detail_id))
      {
          ...
          $heading="Location of ".$asset_details->barcode_number ."&nbsp;(&nbsp;".$assets->asset_name."&nbsp;)&nbsp;";
           ...
           $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li>
                        <a href='".site_url('Dispatch/view/'.$asset_details->asset_id)."'>View Asset Details</a>
                    </li>
                    <li class='active'>".$heading."</li>
                    </ul>";
