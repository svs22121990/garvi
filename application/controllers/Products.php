...
class Products extends CI_Controller
{
  ...
  public function search()
  {
    if ($this->input->post()) {
      ...
      $strUrl = site_url('Products/ajax_manage_page/' . $date);
  ...
  public function index()
  {
    ...
    $this->common_view(site_url('Products/ajax_manage_page'));
  }
  ...
  public function common_view($action, $date = 0)
  {
    ...
    if (!empty($_SESSION[SESSION_NAME]['getMenus'])) {
      ...
      $breadcrumbs = "<ul class='breadcrumb'>
	                ...
	                <li class='active'>Manage Dispatch Note/Bill</li>
	                </ul>";
      ...
      $data = array('dateinfo' => $date, 'breadcrumbs' => $breadcrumbs, 'actioncolumn' => '6', 'ajax_manage_page' => $action, 'heading' => 'Manage Dispatch Note/Bill', 'branch_data' => $branch_data, 'import' => $import, 'importaction' => $importaction, 'download' => $download, 'addPermission' => $add);
  ...
  public function ajax_manage_page($date = 0)
  {
    ...
    $Data = $this->Assets_model->get_datatables($con, $date);
    /*echo"<pre>";
    print_r($Data);exit;*/
    $edit = '';
    $delete = '';
    ...
    foreach ($Data as $row) {
      $btn = '';
      if (!empty($view)) {
        $btn .= '<a href=' . site_url("Products/view/" . $row->id) . ' title="Details" class="btn btn-primary btn-circle btn-sm"><i class="fa fa-eye"></i></a>';
      }

      if (!empty($delete)) {
        $url = base_url() . 'index.php/products/delete/' . $row->id;
    ...
    echo json_encode(array('data' => $arrOutput));
    exit();
  }
  public function export_bill_summary($date = 0)
  {
    ...
    $FileTitle = 'Dispatch Note/Bill';

    $this->load->library('excel');
    //activate worksheet number 1
    $this->excel->setActiveSheetIndex(0);
    //name the worksheet
    $this->excel->getActiveSheet()->setTitle('Dispatch Note-Bill');
    //set cell A1 content with some text
    $this->excel->getActiveSheet()->setCellValue('B1', 'Product Details');

    $this->excel->getActiveSheet()->setCellValue('A3', 'Sr. No.');
    $this->excel->getActiveSheet()->setCellValue('B3', 'Bill No.');
    $this->excel->getActiveSheet()->setCellValue('C3', 'Bill Date');
  ...
  public function pdf_summary($date = 0)
  {
    $con = "p.id<>''";
    $con = "p.id<>''";
    $data['results'] = $this->Assets_model->get_datatables($con, $date);
    //$data['results'] = $query->result();
    $html = $this->load->view('assets/product_pdf2', $data, TRUE);
    $mpdf = new \Mpdf\Mpdf();
    $mpdf->WriteHTML($html);
  ...
  public function export_pdf($id)
  {
    ...
    $html = $this->load->view('assets/product_pdf', $data, TRUE);
    $mpdf = new \Mpdf\Mpdf();
    $mpdf->WriteHTML($html);
  ...
  {
   $results = $this->Assets_model->getAllDetails($id);
    ...
    $this->excel->getActiveSheet()->setCellValue('A3', 'Bill No.');
    $this->excel->getActiveSheet()->setCellValue('B3', (!empty($results) ? $results[0]->bill_no : ""));
    $this->excel->getActiveSheet()->setCellValue('D3', 'Bill Date');
    $this->excel->getActiveSheet()->setCellValue('E3', (!empty($results) ? $results[0]->bill_date : ""));
  ...
  public function addexStock($id)
  {
    ...
    $getAssetData = $this->Assets_model->getAllDetails($id);
    ...
    $this->load->view('assets/addExstock', $data);
  }
  ...
  public function addexStock_action($id)
  {
    ...
    $this->Crud_model->SaveData('assets', $qtydata, "id='" . $id . "'");
  ...
  public function create_action()
  {
    ...
    if ($_POST) {
      ...
      if (count($_POST['sku']) > 1) {
        ...
        $product_purchase_date = date("Y-m-d", strtotime($_POST['product_purchase_date'][0]));
  ...
  public function assetDetails($ast_id, $id, $flag)
  {
    if ($ast_id == '' || $id == '' || $flag == '') {
      ...
      $getAssetData = $this->Assets_model->getAllDetails($ast_id);
      ...
      $action = site_url('Products/addDetailsaction/' . $ast_id . '/' . $id . '/' . $flag);
      $data = array(
        'breadcrumbs' => $breadcrumbs,
        'heading' => $heading,
        'getAssetData' => $getAssetData,
  ...
  {
    $assets = $this->Crud_model->GetData('assets', '', "id='" . $ast_id . "'", '', '', '', '1');
  ...
  public function view($id)
  {
    if ($id == '') {
      ...
      $breadcrumbs = "<ul class='breadcrumb'>
          ...
          <li class=''> 
            <a href='" . site_url('Products') . "'>Manage Products</a>
          </li>
  ...
  public function update($id)
  {
    ...
    $products = $this->Crud_model->GetData('products', "", "id='" . $id . "'", '', '', '', 'row');
    $category_data = $this->Crud_model->GetData('categories', "", "status='Active'", '', 'title asc');
    $asset_type_data = $this->Crud_model->GetData('mst_asset_types', "", "status='Active' and is_delete='No'", '', 'type asc');
    $getAssetData = $this->Crud_model->GetData("assets", "", "product_id='" . $id . "'", "", "", "", "1");
    $catData = $this->Crud_model->GetData('categories', "", "id='" . $getAssetData->category_id . "'", '', '', '', '1');
    ...
    $this->load->view('assets/update_form', $data);
  }
  ...
  {
    $con = "id='" . $id . "'";
    /*$delete = $this->Crud_model->DeleteData('products',$con);
        if($delete){
    ...
    $this->session->set_flashdata('message', '<span class="label label-danger text-center" style="margin-bottom:0px">Asset has been not deleted</span>');
    /*/} */
  ...
  public function log_details($id, $branch_id = '')
  {
    ...
    $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li><a href='" . site_url('Products/index') . "'>Manage Products</a></li>
    ...
    $data = array(
      ...
      'branch_id' => $branch_id,
      'actioncolumns' => '8',
      'ajax_manage_pages' => site_url('Products/ajax_manage_pages/' . $id . '/' . $branch_id),
    );
  ...
  public function getAssetTransferViewAjax($id)
  {
    ...
    foreach ($Data as $row) {
      ...
      $btn = '<a href=' . site_url("Products/viewAssetDetails/" . $row->asset_id . '/' . $row->id) . ' title="Details" class="btn btn-info btn-circle btn-sm"><i class="fa fa-eye"></i></a>';

      $asset_geo_locations = $this->Crud_model->GetData('asset_geo_locations', '', "asset_detail_id='" . $row->id . "'", '', '', '', '1');

      if (!empty($asset_geo_locations)) {

        $btn .= ' | <a href="' . site_url("Products/assetDetailGeoLocation/") . $row->asset_id . '/' . $row->id . '"  class="btn btn-danger btn-circle btn-sm"><i class="fa fa-map-marker"></i></a>';
      }
    ...
    if (!empty($id)) {
      $breadcrumbs = "<ul class='breadcrumb'>
                    <li>
                     ...
                     <li class=''> <a href='" . site_url('Products') . "'>Manage Products</a></li>
                     <li class=''> <a href='" . site_url('Products/view/' . $asset_id) . "'>Manage Products View</a></li>
  ...
  public function assetDetailGeoLocation($asset_id, $asset_detail_id)
  {
    if (!empty($asset_id) && !empty($asset_detail_id)) {
      ...
      $breadcrumbs = "<ul class='breadcrumb'>
                    ...
                    <li>
                        <a href='" . site_url('Products/view/' . $asset_details->asset_id) . "'>View Asset Details</a>
                    </li>
                    <li class='active'>" . $heading . "</li>
                    </ul>";
