  ...
  public function index()
  {
    ...
    {  
        ...
        $breadcrumbs = "<ul class='breadcrumb'>
                        ...
                        <li class='active'>Manage Payment Types</li>
                        </ul>";
            $data = array(
              'breadcrumbs' => $breadcrumbs ,
              'actioncolumn' => '3' ,
              'ajax_manage_page' => site_url('Payment_types/ajax_manage_page') ,
              'button' => 'Create',
              'heading' => 'Manage Payment Types',
              'addPermission'=>$add
            );

            $this->load->view('payment_types/list',$data);
    }
    else
    {
      redirect('Dashboard');
    ...
    public function ajax_manage_page()
    {
      ...
      $view = '';
      $condition="";
      $getData = $this->Payment_types_model->get_datatables($condition);
      foreach($_SESSION[SESSION_NAME]['getMenus'] as $menu)
      { 
        foreach($menu as $row)
        {

            if($row->value=='Payment_types')
            { 
        ...
        $output = array(
          "draw" => $_POST['draw'],
          "recordsTotal" => $this->Payment_types_model->count_all($condition),
          "recordsFiltered" => $this->Payment_types_model->count_filtered($condition),
          "data" => $data,
        );
        ...
        //print_r($_POST);exit;
        $change_status = $this->Crud_model->GetData('payment_types','',"id='".$_POST['id']."'",'','','','row');

        if($change_status->status=='Active')
        {
            $this->Crud_model->SaveData('payment_types',array('status'=>'Inactive'),"id='".$_POST['id']."'");
        }
        else
        {
            $this->Crud_model->SaveData('payment_types',array('status'=>'Active'),"id='".$_POST['id']."'");
        }
        $this->session->set_flashdata('message', '<span class="label label-success text-center" style="margin-bottom:0px;">Status has been changed successfully</span>');
        redirect('Payment_types/index');
    }
     ...
     public function addData()
      {
          $condDuplication = "type='".$this->input->post('type')."'";
          $duplication = $this->Crud_model->GetData('payment_types','', $condDuplication);
          ...
          else
          {
              ...
              $this->Crud_model->SaveData('payment_types',$data);
      ...
      {
          $condDuplication = "type='".$this->input->post('type')."' and id !='".$this->input->post('id')."'";
          $duplication = $this->Crud_model->GetData('payment_types','', $condDuplication);
          if(count($duplication) > 0 )
          {
            echo "1";
          ...
          else
          {
              ...
              $this->Crud_model->SaveData('payment_types', $data, "id='".$this->input->post('id')."'");
              $this->session->set_flashdata('message', 'success');
     ...
     public function getUpdateName()
      {
        $row = $this->Crud_model->GetData('payment_types','',"id='".$_POST['id']."'",'','','','row');   
