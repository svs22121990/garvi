    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><strong>Add New Craft </strong><span id="successEntry" style="color:green"></span></h4>
            </div>
            <div class="modal-body">
                <form method="post" id="cat" onsubmit="return saveData()">
                    <label>Craft Name:</label><span style="color:red">*</span>  <span id="nameError" style="color:red"></span><br>
...
<div class="modal inmodal" id="checkStatus" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated bounceInRight">
            <form method="post" action="<?= site_url('Craft/changeStatus') ?>">
...
<div class="modal fade" id="deleteData" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form method="post" action="<?= site_url('Craft/delete') ?>">
...
<div class="modal fade" id="myModaledit" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title"><strong>Edit Craft </strong><span id="successEditEntry" style="color:green"></span></h4>
            </div>
            <div class="modal-body">
                <form method="post" id="catedit" onsubmit="return updateData()">
                    <label>Craft Name:</label><span style="color:red">*</span>  <span id="titleError" style="color:red"></span><br>
...
<script type="text/javascript">
    ...
    function saveData()
    {
        ...
        if($.trim(name) == "")
        {
            $("#nameError").fadeIn().html("Please enter Craft");
        ...
        $.ajax({
            type : "post",
            url : "<?php echo site_url('Craft/addData'); ?>",
            data : datastring,
            success : function(response)
            {
                ...
                {
                    $('.loader').hide();
                    $("#nameError").fadeIn().html("Craft name already exist");
                ...
                else
                {
                    $(".close").click();
                    $('.loader').hide();
                    $("#successCountryEntry").fadeIn().html("Craft has been Added successfully");
                    //table.draw();
                    table.ajax.reload();
                    $("#successCatEntry").fadeIn().html(" Craft has been Added successfully");
...
<script type="text/javascript">
    ...
    function getEditvalue(rowid)
    {
        ...
        $.ajax({
            type: "POST",
            url: "<?= site_url('Craft/getUpdateName'); ?>",
            data: {id:rowid},
            cache: false,
...
//$("#titleName").val(rowid);
    ...
    function updateData()
    {
        ...
        if($.trim(name) == "")
        {
            $("#titleError").fadeIn().html("Please enter Craft");
        ...
        $.ajax({
            type : "post",
            url : "<?php echo site_url('Craft/updateData') ?>",
            data : datastring,
            success : function(response)
            {
                if(response == 1)
                {
                    $("#titleError").fadeIn().html("Craft name already exist");
                    setTimeout(function(){$("#titleError").fadeOut();},8000);
                }
                else
                {
                    $(".close").click();
                    $("#successCountryEntry").fadeIn().html("Craft has been updated successfully");
                    //table.draw();
                    table.ajax.reload();
                    $("#successCatEntry").fadeIn().html("Craft has been updated successfully");
                    //setTimeout(function() { $("#successCatEntry").fadeOut(); }, 2000);
