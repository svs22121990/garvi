            ...
            <div class="panel panel-default">
                <div class="panel-heading">                                
                    ...
                    <h3 class="panel-title"><span class="msghide"><?= $this->session->userdata('message') <> '' ? $this->session->userdata('message') : ''; ?></span></h3>
                     <h3><span class="label label-success text-center" style="margin-bottom:0px; display: none" id="successUnitEntry"></span></h3>
    ...
    <div class="modal-dialog">     
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><strong>Add Unit Type </strong><span id="successEntry" style="color:green"></span></h4>
        </div>
  ...
  <div class="modal inmodal" id="checkStatus" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated bounceInRight">   
            <form method="post" action="<?= site_url('Unit_types/changeStatus') ?>">       
...
<div class="modal fade" id="deleteData" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">   
            <form method="post" action="<?= site_url('Unit_types/delete') ?>">       
...
<div class="modal fade" id="myModaledit" role="dialog">
    <div class="modal-dialog">     
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title"><strong>Edit Unit Type </strong><span id="successEditEntry" style="color:green"></span></h4>
        </div>
        <div class="modal-body">
         <form method="post" id="unitedit" onsubmit="return updateData()">
            <label>Unit Type Name:</label> <span style="color:red">*</span> <span id="titleError" style="color:red"></span><br>
...
<script type="text/javascript">
    ...
    $('#unitedit').submit(function(event)
  {
    ...
    function saveData()
    {
        ...
        $.ajax({            
          type : "post",
          url : "<?php echo site_url('Unit_types/addData'); ?>",
          data : datastring,
          success : function(response)
          {
            ...
            {
                $(".close").click(); 
                $("#successUnitEntry").fadeIn().html("Unit Type has been Added successfully");
              setTimeout(function(){$("#successUnitEntry").fadeOut();},8000);
                //$("#successCountryEntry").fadeIn().html("Unit Type has been Added successfully");
                 $("#myModal").modal("hide"); 
                  table.draw();
                //setTimeout(function() { $("#successUnitEntry").fadeOut(); }, 2000);   
...
function getEditvalue(rowid)
    {     
        ...
        $.ajax({
          type: "POST",
          url: "<?= site_url('Unit_types/getUpdateName'); ?>",
          data: {id:rowid},
          cache: false,       
...
//$("#titleName").val(rowid);
    ...
    function updateData()
    {
        ...
        if($.trim(name) == "")
        {
              $("#titleError").fadeIn().html("Please enter Unit type");
    ...
    $("#titleError").fadeIn().html("Name can have only alphabets (@,&,*,$,#,! are not allowed)");
        ...
        $.ajax({
          type : "post",
          url : "<?php echo site_url('Unit_types/updateData') ?>",
          data : datastring,
          success : function(response)
          {
            if(response == 1)
            {
              $("#titleError").fadeIn().html("Unit type name already exist");
              setTimeout(function(){$("#titleError").fadeOut();},8000);
            }
            else
            {
             $(".close").click(); 
             $("#successUnitEntry").fadeIn().html("Unit Type has been updated successfully");
              setTimeout(function(){$("#successUnitEntry").fadeOut();},8000);
            // $("#successCountryEntry").fadeIn().html("Unit type has been updated successfully");
