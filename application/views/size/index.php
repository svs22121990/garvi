    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><strong>Add New Size </strong><span id="successEntry" style="color:green"></span></h4>
            </div>
            <div class="modal-body">
                <form method="post" id="cat" onsubmit="return saveData()">
                    <label>Size Name:</label><span style="color:red">*</span>  <span id="nameError" style="color:red"></span><br>
...
<div class="modal inmodal" id="checkStatus" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated bounceInRight">
            <form method="post" action="<?= site_url('Size/changeStatus') ?>">
...
<div class="modal fade" id="deleteData" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form method="post" action="<?= site_url('Size/delete') ?>">
...
<div class="modal fade" id="myModaledit" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title"><strong>Size Color </strong><span id="successEditEntry" style="color:green"></span></h4>
            </div>
            <div class="modal-body">
                <form method="post" id="catedit" onsubmit="return updateData()">
                    <label>Size Name:</label><span style="color:red">*</span>  <span id="titleError" style="color:red"></span><br>
...
<!--    <div class="modal-dialog modal-sm">-->
<!--        <div class="modal-content animated bounceInRight">-->
<!--            <form method="post" action="--><?//= site_url('Color/import')?><!--" method="post" enctype="multipart/form-data" onsubmit="return checkXcel()">-->
<!--                <div class="modal-header">-->
<!--                    <span style="font-size:20px">Upload Color</span>-->
<!---->
<!--                </div>-->
<!--                <div class="modal-body" style="padding-top: 3%">-->
<!--                    <a download class="pull-right" href="--><?php //echo base_url(); ?><!--uploads/color.xls" style="font-size:10px">Download Sample Format</a>-->
...
<script type="text/javascript">
    ...
    function saveData()
    {
        ...
        if($.trim(name) == "")
        {
            $("#nameError").fadeIn().html("Please enter Size");
        ...
        $.ajax({
            type : "post",
            url : "<?php echo site_url('Size/addData'); ?>",
            data : datastring,
            success : function(response)
            {
                ...
                {
                    $('.loader').hide();
                    $("#nameError").fadeIn().html("Size name already exist");
                ...
                else
                {
                    $(".close").click();
                    $('.loader').hide();
                    $("#successCountryEntry").fadeIn().html("Size has been Added successfully");
                    //table.draw();
                    table.ajax.reload();
                    $("#successCatEntry").fadeIn().html(" Size has been Added successfully");
...
<script type="text/javascript">
    ...
    function getEditvalue(rowid)
    {
        ...
        $.ajax({
            type: "POST",
            url: "<?= site_url('Size/getUpdateName'); ?>",
            data: {id:rowid},
            cache: false,
...
//$("#titleName").val(rowid);
    ...
    function updateData()
    {
        ...
        if($.trim(name) == "")
        {
            $("#titleError").fadeIn().html("Please enter Size");
        ...
        $.ajax({
            type : "post",
            url : "<?php echo site_url('Size/updateData') ?>",
            data : datastring,
            success : function(response)
            {
                if(response == 1)
                {
                    $("#titleError").fadeIn().html("Size name already exist");
                    setTimeout(function(){$("#titleError").fadeOut();},8000);
                }
                else
                {
                    $(".close").click();
                    $("#successCountryEntry").fadeIn().html("Size has been updated successfully");
                    //table.draw();
                    table.ajax.reload();
                    $("#successCatEntry").fadeIn().html("Size has been updated successfully");
                    //setTimeout(function() { $("#successCatEntry").fadeOut(); }, 2000);
