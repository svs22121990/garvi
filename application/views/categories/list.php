    ...
    <div class="modal-dialog">     
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><strong>Add New Category </strong><span id="successEntry" style="color:green"></span></h4>
        </div>
        <div class="modal-body">
         <form method="post" id="cat" onsubmit="return saveData()">
            <label>Category Name:</label><span style="color:red">*</span>  <span id="nameError" style="color:red"></span><br>
  ...
  <div class="modal inmodal" id="checkStatus" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated bounceInRight">   
            <form method="post" action="<?= site_url('Categories/changeStatus') ?>">       
...
<div class="modal fade" id="myModaledit" role="dialog">
    <div class="modal-dialog">     
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title"><strong>Edit Category </strong><span id="successEditEntry" style="color:green"></span></h4>
        </div>
        <div class="modal-body">
         <form method="post" id="catedit" onsubmit="return updateData()">
            <label>Category Name:</label><span style="color:red">*</span>  <span id="titleError" style="color:red"></span><br>
...
<div class="modal inmodal" id="uploadData" data-modal-color="lightblue" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated bounceInRight">   
            <form method="post" action="<?= site_url('Categories/import')?>" method="post" enctype="multipart/form-data" onsubmit="return checkXcel()">  
                <div class="modal-header">
                        <span style="font-size:20px">Upload Categories</span>

                    </div>     
                <div class="modal-body" style="padding-top: 3%">
                 <a download class="pull-right" href="<?php echo base_url(); ?>uploads/categories.xls" style="font-size:10px">Download Sample Format</a>
 ...
 $('#cat').submit(function(event)
    ...
    $('#catedit').submit(function(event)
  {
    ...
    function saveData()
    {
        ...
        if($.trim(name) == "")
        {
              $("#nameError").fadeIn().html("Please enter Category");
        ...
        $.ajax({            
          type : "post",
          url : "<?php echo site_url('Categories/addData'); ?>",
          data : datastring,
          success : function(response)
          {
            ...
            {
              $('.loader').hide();
              $("#nameError").fadeIn().html("Category name already exist");
...
function getEditvalue(rowid)
    {  
      $('.loader').show();
        ...
        $.ajax({
          type: "POST",
          url: "<?= site_url('Categories/getUpdateName'); ?>",
          data: {id:rowid},
          cache: false,       
...
//$("#titleName").val(rowid);
    ...
    function updateData()
    {
        ...
        if($.trim(name) == "")
        {
              $("#titleError").fadeIn().html("Please enter Category");
    ...
    $("#titleError").fadeIn().html("Name can have only alphabets (@,&,*,$,#,! are not allowed)");
        ...
        $.ajax({
          type : "post",
          url : "<?php echo site_url('Categories/updateData') ?>",
          data : datastring,
          success : function(response)
          {
            if(response == 1)
            {
              $("#titleError").fadeIn().html("Category name already exist");
              setTimeout(function(){$("#titleError").fadeOut();},8000);
            }
            else
            {
             $(".close").click(); 
             $("#successCountryEntry").fadeIn().html("Category has been updated successfully");
             //table.draw();
            table.ajax.reload();
              $("#successCatEntry").fadeIn().html("Category has been updated successfully");
                //setTimeout(function() { $("#successCatEntry").fadeOut(); }, 2000); 
